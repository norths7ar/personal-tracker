...
export function LedgerPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return allRows.filter((row) => {
      ...
      if (categoryFilter !== "全部" && row.category !== categoryFilter)
        return false;
      if (subcategoryFilter !== "全部" && row.subcategory !== subcategoryFilter)
        return false;
  ...
  const subcategoryOptions = useMemo(() => {
    return unique(
      allRows
        ...
        .filter(
          (row) => categoryFilter === "全部" || row.category === categoryFilter,
        )
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="space-y-4 rounded-lg border border-neutral-200 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-[140px_180px_180px_minmax(200px,1fr)]">
          ...
          <FilterSelect
            ...
            onChange={(value) => updateFilters({ subcategory: value })}
          />
        ...
        <div className="flex flex-wrap items-end gap-3 border-t border-neutral-100 pt-4">
          ...
          <div className="flex flex-1 flex-wrap items-center gap-2">
            <Button variant="outline" onClick={() => selectMonth(0)}>
              本月
            </Button>
            <Button variant="outline" onClick={() => selectMonth(-1)}>
              上月
            </Button>
            <Button
              variant="outline"
          ...
          <table className="w-full min-w-[960px] border-collapse text-sm">
            ...
            <tbody>
              {transactions.isPending ? (
                ...
                <TableMessage columns={columns.length} danger>
                ...
                table.getRowModel().rows.map((row) => (
                  <tr
                    ...
                    {...rowEvents(row.original.id, () =>
                      setEditing(row.original),
                    )}
                    className={cn(
                      "cursor-pointer border-b border-neutral-100 last:border-0 hover:bg-neutral-50",
                      row.getIsSelected() && "bg-amber-50 hover:bg-amber-50",
                    )}
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="max-w-80 truncate px-3 py-2.5"
                      >
                        {flexRender(
      ...
      <RecordActions
        ...
        onEdit={() =>
          selectedRecord ? setEditing(selectedRecord) : setBulkEditing(true)
        }
                ...
                current.map((row) =>
                  selectedIds.includes(row.id)
                    ? {
                        ...
                        ...changes,
                        reviewed:
                          "category" in changes || "subcategory" in changes
                            ? true
                            : row.reviewed,
                      }
                    : row,
...
function BulkTransactionEditor({
  ...
  onClose: () => void;
  onSaved: (
    changes: Partial<Pick<Transaction, "category" | "subcategory" | "notes">>,
  ) => void;
}) {
  ...
  const initialCategory =
    commonValue(transactions.map((transaction) => transaction.category)) ?? "";
  const initialSubcategory =
    commonValue(transactions.map((transaction) => transaction.subcategory)) ??
    "";
  const initialNotes =
    commonValue(transactions.map((transaction) => transaction.notes)) ?? "";
  ...
  const subcategoryOptions =
    categories[transactionType ?? ""]?.[category] ?? [];
  const changes: Partial<
    Pick<Transaction, "category" | "subcategory" | "notes">
  > = {};
  if (changeCategory) {
      ...
      <DialogContent layout="center">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <LockedField
              label="金额"
              value={`合计 ${formatMoney(totalAmount)}`}
            />
          </div>
...
function LockedField({ label, value }: { label: string; value: string }) {
  return (
    <FormField label={label}>
      <Input
        className="border-neutral-200 bg-neutral-100 text-neutral-500 disabled:opacity-100"
        value={value}
        disabled
        readOnly
      />
...
function TransactionEditor({
  ...
  const [tab, setTab] = useState<
    "edit" | "amortization" | "recurring" | "refund"
  >("edit");
          ...
          <>
              ...
              {(
                ...
                <button
                  ...
                  className={cn(
                    ...
                    tab === value && "bg-white font-medium shadow-sm",
                  )}
            ...
            {tab === "edit" && (
              ...
              >
                ...
                <FormField label="备注">
                  <textarea
                    className="min-h-24 w-full rounded-md border border-neutral-300 p-3 text-sm outline-none focus:border-neutral-500 focus:ring-2 focus:ring-neutral-200"
                    {...form.register("notes")}
                  />
                </FormField>
                {update.isError && (
            ...
            {tab === "amortization" && (
              <AmortizationForm transaction={transaction} onSaved={onSaved} />
            )}
            {tab === "recurring" && (
              <RecurringForm transaction={transaction} onSaved={onRefresh} />
            )}
            {tab === "refund" && (
              <RefundForm
...
function AmortizationForm({
  ...
  const [months, setMonths] = useState(
    String(transaction.amortization_months ?? 12),
  );
  const [startMonth, setStartMonth] = useState(
    (transaction.amortization_start ?? transaction.date).slice(0, 7),
  );
  const update = useMutation({
    mutationFn: () =>
      api.updateTransaction(transaction.id, {
  ...
  return (
    ...
    >
      ...
      <Button
        className="w-full"
        disabled={update.isPending || Number(months) < 2}
      >
        {update.isPending ? "保存中…" : "保存摊销"}
      </Button>
    </form>
  );
}
...
function RefundForm({
  ...
  const refunded = transactions
    .filter((item) => item.refund_for_id === transaction.id)
    .reduce((sum, item) => sum + item.amount, 0);
  const remaining = Math.max(0, transaction.amount - refunded);
  const [entry, setEntry] = useState<RefundCreate>({
    description: `${transaction.description} 退款`,
    amount: remaining,
    date: formatDate(new Date()),
  });
  const [idempotencyKey] = useState(() => crypto.randomUUID());
  const create = useMutation({
    mutationFn: () => api.createRefund(transaction.id, entry, idempotencyKey),
    onSuccess: onSaved,
  });
  if (transaction.type !== "支出")
    return <Info>只有支出记录可以关联退款。</Info>;
  return (
    <form
      className="space-y-4"
      onSubmit={(event) => {
    ...
    >
      <p className="text-sm text-neutral-500">
        已关联退款 {formatMoney(refunded)}，剩余可退 {formatMoney(remaining)}。
      </p>
      <FormField label="退款描述">
        <Input
...
function RecurringForm({
  ...
  const create = useMutation({
    mutationFn: () => {
      ...
      const entry: SubscriptionCreate = {
        ...
        renewal_anchor_day:
          mode === "same_day" ? Number(nextDate.slice(8, 10)) : null,
      };
  ...
  if (transaction.type !== "支出")
    return <Info>只有支出记录可以设为周期性付款。</Info>;
  if (transaction.subscription_id != null)
    return <Info>这笔支出已经关联周期性付款。</Info>;
  return (
    <form
      className="space-y-4"
      onSubmit={(event) => {
    ...
    >
      <p className="text-sm text-neutral-500">
        用这笔支出作为首次付款，后续到期时再确认入账。
      </p>
      <FormField label="描述">
        <Input value={name} onChange={(event) => setName(event.target.value)} />
      </FormField>
      <FormField label="下次付款日">
        <Input
          type="date"
          value={nextDate}
      ...
      <div className="grid grid-cols-2 gap-3">
        <FormField label="续费方式">
          <select
            className={selectClass}
            value={mode}
            onChange={(event) => {
              const value = event.target.value as "same_day" | "fixed_days";
              setMode(value);
              setInterval(value === "same_day" ? "1" : "30");
            }}
          >
        ...
        <FormField label={mode === "same_day" ? "间隔月数" : "间隔天数"}>
          <Input
            type="number"
            min="1"
            max={mode === "same_day" ? "120" : "730"}
            value={interval}
      ...
      <Button
        className="w-full"
        disabled={create.isPending || !name.trim() || Number(interval) < 1}
      >
        {create.isPending ? "创建中…" : "创建周期性付款"}
      </Button>
    </form>
  );
}
