...
export function PendingPage() {
  ...
  const bulkUpdate = useMutation({
    mutationFn: () =>
      api.updateTransactions(checkedIds, {
      ...
      {Boolean(checkedIds.length) && (
        ...
        >
          <p className="text-sm font-medium">
            已选择 {checkedIds.length} 笔，统一确认分类为：
          </p>
          <div className="flex flex-wrap items-end gap-3">
            <Field label="批量主类别">
              ...
              >
                ...
                {Object.keys(categories.data?.支出 ?? {}).map((category) => (
            ...
            {bulkSubcategories.length > 0 && (
              <Field label="批量子类别">
                ...
                >
                  <option value="">请选择</option>
                  {bulkSubcategories.map((category) => (
                    <option key={category}>{category}</option>
                  ))}
                </select>
              </Field>
            ...
            <Button
              disabled={
                ...
                update.isPending ||
                !bulkCategory ||
                (bulkSubcategories.length > 0 && !bulkSubcategory)
              }
            >
              {bulkUpdate.isPending
                ? "保存中…"
                : `确认 ${checkedIds.length} 笔分类`}
            </Button>
            <Button
              type="button"
              variant="ghost"
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]">
            ...
            <table className="w-full text-sm">
              <thead className="bg-neutral-50 text-left text-xs text-neutral-500">
                <tr>
                  <th className="p-3">
                    <Checkbox
                      aria-label="选择全部待处理支出"
                      disabled={bulkUpdate.isPending || update.isPending}
                      checked={
                        checkedIds.length === pending.data.length ||
                        (checkedIds.length > 0 && "indeterminate")
                      }
                      onCheckedChange={(checked) =>
                        setCheckedIds(
                          checked ? pending.data.map((row) => row.id) : [],
                        )
                      }
                    />
                  </th>
                  ...
                  >
                    ...
                    <td className="p-3">
                      <button
                        ...
                        aria-pressed={row.id === effectiveSelectedId}
                        onClick={() => setSelectedId(row.id)}
                      >
                        {row.description}
                      </button>
          ...
          {selected && (
            <PendingEditor
              ...
              error={update.error}
              onSave={(changes) => update.mutate({ id: selected.id, changes })}
            />
          )}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <Field label="主类别">
        ...
        >
          {[...Object.keys(categories), pendingCategory].map((category) => (
            <option key={category}>{category}</option>
          ))}
        </select>
      </Field>
